...
def generar(con, modelo, cfg: dict, fecha: str) -> dict:
    ...
    # el "parlay corto" toma las 3 mejores y el "largo" usa todas (todos los juegos).
    segura = sorted(segura, key=lambda r: r["prob"], reverse=True)
    arriesgada = sorted(arriesgada, key=lambda r: r["prob"], reverse=True)
    sonador = sorted(sonador, key=lambda r: r["prob"], reverse=True)

...
Los umbrales están calibrados para clubes: un grande europeo acumula 400+
partidos en 8 temporadas, mientras que Slavia, Shakhtar, Slovan y Sabah —cuyas
ligas ESPN no cubre— solo suman sus partidos europeos y caerán en nivel bajo o
medio. Eso es exactamente lo que queremos que la interfaz muestre.
"""

from __future__ import annotations


def nivel_por_datos(n_min: int, umbral_alto: int = 120, umbral_medio: int = 40) -> str:
...
def confianza_partido(con, local: str, visit: str, cfg: dict) -> dict:
    ...
    desde = cfg.get("datos", {}).get("desde", "2018-07-01")
    ...
    nivel = nivel_por_datos(
        n_min,
        umbrales.get("alto", 120),
        umbrales.get("medio", 40),
    )
    return {
        "n_local": n_local,
        "n_visit": n_visit,

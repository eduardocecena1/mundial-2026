...
> ✅ **Se actualiza sola cada día.** La primera vez que alguien abre la web cada día,
> la app refresca desde ESPN los resultados de ayer y los partidos de hoy. No tienes
> que hacer nada: abres la web el día de la jornada y ya está todo.

> 📦 **La base `data/clubes.db` (13 MB) SÍ se sube al repo**, a diferencia del resto de
> datos. Bajar las 21 competiciones desde cero tarda 10-20 minutos y Streamlit Cloud
> no aguanta eso al arrancar. Con la base versionada la app abre al instante y solo
> descarga la temporada en curso. Si algún día la borras, la app la reconstruye sola
> (pero tardará).
...
1. En GitHub, pulsa **New repository**, ponle nombre (ej. `champions-picks`), déjalo
   **Private** si quieres, y crea.
2. GitHub te muestra unos comandos. En la terminal, dentro de la carpeta del proyecto:
   ```bash
   git remote add origin https://github.com/TU_USUARIO/champions-picks.git
   git branch -M main
...
gh repo create champions-picks --private --source=. --push
```
...
3. Rellena:
   - **Repository:** `TU_USUARIO/champions-picks`
   - **Branch:** `main`
...
¡Listo! Te queda una URL tipo `https://champions-picks-xxxx.streamlit.app`.
...
- **No subas el archivo `.env`** (tu key de API-Football). Ya está en `.gitignore`,
  así que no se sube. La web no necesita ninguna key para funcionar: todo sale del
  API público de ESPN.
- **La base `data/clubes.db` sí se sube**; el resto de `data/` (cachés y modelos
  entrenados) no, porque se regeneran solos.
- Cuando cambies de temporada, acuérdate de actualizar `competicion.temporada` en
  `config.yaml` y de hacer `git add data/clubes.db` tras la descarga.
